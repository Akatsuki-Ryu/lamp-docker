...
0
|| checktplrefresh('./template/default/forum/post_infloat.htm', './template/default/common/seditor.htm', 1726579610, '1', './data/template/2_1_forum_post_infloat.tpl.php', './template/default', 'forum/post_infloat')
|| checktplrefresh('./template/default/forum/post_infloat.htm', './template/default/forum/seccheck_post.htm', 1726579610, '1', './data/template/2_1_forum_post_infloat.tpl.php', './template/default', 'forum/post_infloat')
|| checktplrefresh('./template/default/forum/post_infloat.htm', './template/default/common/seccheck.htm', 1726579610, '1', './data/template/2_1_forum_post_infloat.tpl.php', './template/default', 'forum/post_infloat')
...
<?php if($_GET['action'] == 'newthread' && $modnewthreads) { ?><span class="needverify">需审核</span><?php } if($_GET['action'] == 'reply' && !empty($modnewreplies)) { ?><span class="needverify">需审核</span><?php } ?>
<span>
...
<?php if(!$sortid) { ?><option value="0">分类信息</option><?php } if(isset($threadsorts['types']) && is_array($threadsorts['types'])) foreach($threadsorts['types'] as $tsortid => $name) { if(!empty($modelid) && $threadsorts['modelid'][$tsortid] == $modelid || empty($modelid)) { ?>
...
<?php } if($_GET['action'] != 'reply') { ?>
<span><input name="subject" id="subject" class="px" value="<?php echo $postinfo['subject'];?>" style="width: 25em" /></span>
<?php } else { ?>
<span id="subjecthide" class="z">RE: <?php echo $thread['subject'];?> [<a href="javascript:;" onclick="display('subjecthide');display('subjectbox');">修改</a>]</span>
...
<div class="bar">
<span class="y">
<a href="forum.php?mod=post&amp;action=<?php echo $_GET['action'];?>&amp;fid=<?php echo $_G['fid'];?>&amp;extra=<?php echo $extra;?><?php if($_GET['action'] == 'reply') { ?>&amp;tid=<?php echo $_G['tid'];?><?php if(!empty($_GET['reppost'])) { ?>&amp;reppost=<?php echo $_GET['reppost'];?><?php } if(!empty($_GET['repquote'])) { ?>&amp;repquote=<?php echo $_GET['repquote'];?><?php } if(!empty($page)) { ?>&amp;page=<?php echo $page;?><?php } } if(!empty($stand)) { ?>&amp;stand=<?php echo $stand;?><?php } ?>" onclick="switchAdvanceMode(this.href);doane(event);">高级模式</a>
...
<div class="area">
<textarea rows="7" cols="80" name="message" id="postmessage" onKeyDown="seditor_ctlent(event, '$(\'postsubmit\').click();')" class="pt"><?php echo $message;?></textarea>
</div>
</div>
...
<a href="javascript:;" id="custominfo" class="y" style="margin-left:5px"><i class="fico-info fic4 fc-p" alt="帮助"></i></a>
<?php } ?>
<a href="home.php?mod=spacecp&amp;ac=credit&amp;op=rule&amp;fid=<?php echo $_G['fid'];?>" class="y" target="_blank">本版积分规则</a>
<button type="submit" id="postsubmit" class="pn pnc z" value="true"<?php if(!$seccodecheck && !$secqaacheck) { ?> onmouseover="checkpostrule('seccheck_<?php echo $_GET['action'];?>', 'ac=<?php echo $_GET['action'];?>&infloat=yes&handlekey=<?php echo $_GET['handlekey'];?>');this.onmouseover=null"<?php } ?> name="<?php if($_GET['action'] == 'newthread') { ?>topicsubmit<?php } elseif($_GET['action'] == 'reply') { ?>replysubmit<?php } ?>"><span><?php if($_GET['action'] == 'newthread') { ?>发表帖子<?php } elseif($_GET['action'] == 'reply') { ?>参与/回复主题<?php } ?></span></button>
...
ajaxget('forum.php?mod=viewthread&tid=<?php echo $_G['tid'];?>&viewpid=' + pid<?php if(!empty($_GET['from'])) { ?> + '&from=<?php echo $_GET['from'];?>'<?php } ?>, 'post_new', 'ajaxwaitid', '', null, 'appendreply()');
if(replyreload) {
var reloadpids = replyreload.split(',');
for(var i = 1;i < reloadpids.length;i++) {
ajaxget('forum.php?mod=viewthread&tid=<?php echo $_G['tid'];?>&viewpid=' + reloadpids[i]<?php if(!empty($_GET['from'])) { ?> + '&from=<?php echo $_GET['from'];?>'<?php } ?>, 'post_' + reloadpids[i]);
}
}
} else {
