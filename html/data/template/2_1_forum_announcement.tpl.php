...
<div class="mn">
<div class="bm bw0">
<div id="annofilter"></div><?php if(isset($announcelist) && is_array($announcelist)) foreach($announcelist as $ann) { ?><div id="announce<?php echo $ann['id'];?>_c" class="umh<?php if($messageid != $ann['id']) { ?> umn<?php } ?>">
...
<ul id="annonav">
<li class="cl<?php if(empty($_GET['m'])) { ?> a<?php } ?>"><a href="forum.php?mod=announcement">全部</a></li><?php if(isset($months) && is_array($months)) foreach($months as $month) { ?><li class="cl<?php if($_GET['m'] == $month[0].$month[1]) { ?> a<?php } ?>"><a href="forum.php?mod=announcement&amp;m=<?php echo $month[0].$month[1];?>"><?php echo $month[0];?> 年 <?php echo $month[1];?> 月</a></li>
<?php } ?>
</ul>
</div>
</div>
