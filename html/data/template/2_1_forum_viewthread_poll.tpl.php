...
<?php if($isimagepoll) { $i = 0;?><tr><?php if(isset($polloptions) && is_array($polloptions)) foreach($polloptions as $key => $option) { $i++;?><?php $imginfo=$option['imginfo'];?><td valign="bottom" id="polloption_<?php echo $option['polloptionid'];?>" width="25%">
<div class="polltd cl">
<?php if($imginfo) { ?>
...
<?php } else { ?>
<a href="javascript:;" title=""><div class="nophoto"></div></a>
<?php } ?>
<p class="mtn mbn xi2">
...
<?php } else { if(isset($polloptions) && is_array($polloptions)) foreach($polloptions as $key => $option) { ?><tr<?php if($visiblepoll) { ?> class="ptl"<?php } ?>>
<?php if($_G['group']['allowvote']) { ?>
