...
<?php } if($_G['setting']['verify']['enabled']) { if(isset($_G['setting']['verify']) && is_array($_G['setting']['verify'])) foreach($_G['setting']['verify'] as $vid => $verify) { if($verify['available'] && $verify['showicon']) { if($space['verify'.$vid] == 1) { ?>
...
<?php if($profiles) { ?>
<ul class="card_info"><?php if(isset($profiles) && is_array($profiles)) foreach($profiles as $value) { ?><li>
...
<?php } ?>
<a <?php if(helper_access::check_module('forum')) { ?>href="forum.php?mod=modcp&amp;action=thread&amp;op=post&amp;do=search&amp;searchsubmit=1&amp;users=<?php echo $encodeusername;?>" target="_blank" <?php } ?>class="xi1">管理帖子</a>
<?php } ?>
