...
class helper_form {
	...
	public static function censor($message, $modword = NULL, $return = FALSE, $modasban = TRUE) {
		...
		// check if the message is a long sentence in english or not 

		// Check if the message is a long sentence in English
		// $letterCount = strlen(preg_replace('/[^a-zA-Z]/', '', $message));
		// $letterCount = preg_match_all("/[xB0-xF7][xA1-xFE]/",$message,$regs);
		// showmessage($letterCount);
		// if ($letterCount<=20) {
			// the content is not long enough, so it must suppose to be in chinese
			// if(preg_match_all("/[xB0-xF7][xA1-xFE]/",$message,$regs)<=3) showmessage(" <b><font color=white>请老板还是发一些有意义的中文内容吧，鞠躬!</font></b>");
		// }
